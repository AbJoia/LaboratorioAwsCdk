...
export class TodoNotifyStack extends cdk.Stack {
    constructor(scope: Construct, id:string, props: TodoNotifyStackProps){
        ...
        props.snsTopic.addSubscription(new snsSubscription.SqsSubscription(notifyQueue))
        notifyQueue.grantConsumeMessages(notifyEventFunction);

        notifyEventFunction.addEventSource(new lambdaEventSource.SqsEventSource(notifyQueue, {
            batchSize: 8,
            enabled: true,
